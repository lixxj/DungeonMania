...
public class App implements SparkApplication {
    ...
    public void init() {
        ...
        // Parameters: { xStart: int, yStart: int, xEnd: int, yEnd: int, configName: String }
        // Return Type: { DungeonResponse }
        Spark.post("/api/game/new/generate/", "application/json", (request, response) -> {
            int xStart = Integer.valueOf(request.queryParams("xStart"));
            int yStart = Integer.valueOf(request.queryParams("yStart"));
            int xEnd = Integer.valueOf(request.queryParams("xEnd"));
            int yEnd = Integer.valueOf(request.queryParams("yEnd"));
            String configName = request.queryParams("configName");
            
            return callUsingSessionAndArgument(request, (dmc) -> dmc.generateDungeon(xStart, yStart, xEnd, yEnd, configName));
        }, gson::toJson);

        // Parameters: { ticks: int }
        // Return Type: { DungeonResponse }
        Spark.post("/api/game/new/rewind/", "application/json", (request, response) -> {
            int tick = Integer.valueOf(request.queryParams("tick"));
            return callUsingSessionAndArgument(request, (dmc) -> dmc.rewind(tick));
        }, gson::toJson);
